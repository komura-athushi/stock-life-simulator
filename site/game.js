...
function initializeGame(data) {
  ...
  function render() {
    ...
    $('goals').innerHTML = '<div class="goal-grid">' + data.config.goals.map(g => {
      ...
      return '<article class="goal ' + (passed ? 'passed' : current && !data.config.goals.some(x => x.month >= s.month && x.month < g.month) ? 'selected' : '') + '"><small>' + date(g.month) + '末 ' + (passed ? '✓ 達成' : g.month < s.month || ended ? '未達成' : '') + '</small><strong>' + yen(g.amount) + '</strong><span>' + esc(g.title) + '</span><progress max="' + g.amount + '" value="' + (passed ? g.amount : Math.min(total, g.amount)) + '" aria-label="' + esc(g.title) + 'の達成度"></progress>' + (current && !ended ? '<small>期限まで' + (g.month - s.month + 1) + 'か月 / あと' + yen(Math.max(0, g.amount - total)) + '</small>' : '') + '</article>';
    }).join('') + '</div>';
    $('reward').hidden = !s.rewardPending;
    $('reward').innerHTML = '<h2>目標達成！ ボーナスを選ぼう</h2><p>達成ボーナスは2種類からひとつ。未達成でも24か月まで続けられます。</p><div class="cards"><button data-reward="dividend">複利を育てる<br>配当倍率＋0.10</button><button data-reward="network">情報網を広げる<br>毎月の調査pt＋2</button></div>';
    ...
    $('goals').innerHTML = '<div class="chapter-goal"><div><small>' + date(chapter.month) + '末までに</small><strong>' + yen(chapter.amount) + '</strong></div><div><span>' + esc(chapter.title) + '</span><p>' + (ended ? '今回の挑戦の記録' : chapter.month === s.month ? '今月末にボーナス判定。未達成でも続けられます。' : 'あと' + (chapter.month - s.month + 1) + 'か月 / 達成でボーナス（未達成でも継続）') + '</p></div></div><details class="roadmap"><summary>2年間のボーナス目標を見る</summary>' + roadmap + '</details>';
  ...
  function settleWithScene() {
    ...
    const after = E.assets(s), clear = s.result === 'clear', missed = data.config.goals.some(g => g.month === before.month && !s.goalsPassed.includes(g.month));
    const final = { label: date(before.month) + '・決算', mood: clear || s.rewardPending ? 'gold' : 'calm', art: 'city', icon: clear ? '♛' : s.rewardPending ? '✦' : '◎', title: clear ? '2年間をクリア。あなたの歩みが、ここに。' : s.rewardPending ? '目標達成！ ボーナスを選ぼう。' : missed ? '今回は目標未達。挑戦は続く。' : after > total ? '今月の一手が、実を結んだ。' : after < total ? 'こんな月もある。次の一手を考えよう。' : '次のチャンスを、待とう。', text: s.rewardPending ? '目標達成のボーナスをひとつ選ぼう。' : clear ? '得意を育てて歩んだ、あなたの2年間。' : missed ? '今回のボーナスはありません。次の目標へ進もう。' : '市場が眠り、また新しい月が来る。', value: after, from: total, rows: [['今月の資産増減', (after >= total ? '+' : '') + yen(after - total)], ['配当', '+' + yen(s.stats.dividends - before.stats.dividends)]], word: clear ? 'DREAM ACHIEVED' : 'YOUR BALANCE', close: s.rewardPending ? '報酬を選ぶ →' : clear ? '記録を振り返る' : '次の月へ →' };
    scenes.push(final);
  ...
  document.addEventListener('click', event => {
    ...
    if (d.reward && perform(() => E.reward(s, d.reward), false)) theatre.play([{ label: '目標突破の贈り物', mood: 'gold', art: 'stars', icon: '✦', title: '目標達成のボーナスを獲得！', text: d.reward === 'dividend' ? '配当倍率 ＋0.10' : '毎月の調査pt ＋2', word: 'A NEW CHAPTER' }]);
