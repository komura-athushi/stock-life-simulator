...
async function boot() {
  ...
  try {
    ...
    const entries = await Promise.all(['companies', 'events', 'config'].map(async name => {
      ...
      if (!response.ok) throw Error(name + '.json の取得を再試行してください（HTTP ' + response.status + '）。');
    ...
    controller.abort();
    message.textContent = '市場への接続を再試行してください。' + (error.name === 'AbortError' ? '通信がタイムアウトしました。' : error.message);
