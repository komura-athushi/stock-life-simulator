...
(async()=>{
 ...
 try{
  const page=await browser.newPage({viewport:{width:1440,height:1000},reducedMotion:'reduce'}),errors=[],key='kabugurashi-plans-v3:/';
  ...
  async function inject(s){E.restore(JSON.stringify(s),data);await page.evaluate(({key,raw})=>localStorage.setItem(key,raw),{key,raw:JSON.stringify(s)});await page.reload();await page.locator('#play').waitFor({state:'visible'});}
  await page.goto('http://127.0.0.1:'+server.address().port);await page.locator('[data-career="trader"]').waitFor();
  ...
  await page.locator('[data-career="trader"]').click();await finish();
  ...
  assert.equal((await saved()).actions,1);assert.equal((await saved()).researchPoints,4);
  await page.locator(selector+' [data-inspect]').click();await finish();
  assert.equal((await saved()).actions,1);assert.equal((await saved()).researchPoints,1);assert.equal(await page.locator(selector+' .success-chance').count(),1);
  ...
  const waiting=E.createGame(data,31);E.start(waiting,data,'wealthy');waiting.plans[0].dueMonth=3;E.inspect(waiting,data,waiting.plans[0].uid);
  ...
  const losing=E.createGame(data,42);E.start(losing,data,'trader');
  ...
  const winning=E.createGame(data,43);E.start(winning,data,'wealthy');winning.cash=1e9;
  ...
  await page.locator('#reset').click();await page.locator('#confirmRestart').click();await page.locator('[data-career="influencer"]').click();await finish();
